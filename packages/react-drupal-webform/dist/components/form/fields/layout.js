...
const renderLayout = (props) => {
    const { fieldKey, field, components, watchedValues, classNamePrefix, unstyled, disableActionButtonWhenInvalid = false, } = props;
    ...
    return (_createElement(LayoutComponent, { ...props, fieldKey: fieldKey, key: fieldKey }, childKeys.map((childKey, index) => {
        ...
        if (!shouldFieldBeVisible(childKey, field, values))
            return null;
        return (_jsx(FormFieldRendered, { fieldKey: childKey, index: index, field: child, classNamePrefix: classNamePrefix, components: components, isMultiStep: false, watchedValues: watchedValues, unstyled: unstyled, disableActionButtonWhenInvalid: disableActionButtonWhenInvalid }, childKey));
    })));
};
