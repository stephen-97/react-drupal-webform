...
export const Action = ({ field, innerProps, className }) => {
    const { formState } = useFormContext();
    const { isSubmitting, isValid } = formState;
    return (_jsxs("button", { type: "submit", disabled: !isValid || isSubmitting, className: cn(styles.button, className, ...(field?.['#attributes']?.class ?? [])), ...innerProps, children: [isSubmitting && _jsx(Loader, {}), field?.['#submit__label'] ?? 'Submit'] }));
};
